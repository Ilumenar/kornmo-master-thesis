...
def raw_frost_readings_to_file(from_date, to_date, growth_season, weather_type, client_id):
    ...
    if weather_type == WEATHER_TYPES.PRECIPITATION:
        ...
        download_raw_weather_data('precipitation', from_date, to_date, growth_season, frost_sources,
                                  precipitation_frost_elements, precipitation_time_resolution, client_id)

    elif weather_type == WEATHER_TYPES.TEMPERATURE:
        temperature_frost_elements = 'air_temperature'
        temperature_time_resolutions = ''
        download_raw_weather_data('temperature', from_date, to_date, growth_season, frost_sources,
                                  temperature_frost_elements, temperature_time_resolutions, client_id)

    elif weather_type == WEATHER_TYPES.DAYDEGREE0:
        temperature_frost_elements = 'integral_of_excess(mean(air_temperature P1D) P1D 0.0)'
        temperature_time_resolutions = ''
        download_raw_weather_data('daydegree0', from_date, to_date, growth_season, frost_sources,
                                  temperature_frost_elements, temperature_time_resolutions, client_id)

    elif weather_type == WEATHER_TYPES.DAYDEGREE5:
        temperature_frost_elements = 'integral_of_excess(mean(air_temperature P1D) P1D 5.0)'
        temperature_time_resolutions = ''
