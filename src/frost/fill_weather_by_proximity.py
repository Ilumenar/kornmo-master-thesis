...
def assign_to_farmer_and_fill_by_proximity(start_date, end_date, weather_type):
	...
	stations_df = pd.read_csv('../../../kornmo-data-files/raw-data/weather-data/frost_sources.csv')
	...
	# Each station with its information and weather value for each day in that year.
	...
	for index, farmer in farmers.iterrows():
		...
		# Find the closest station, order by distance and fill, keeping the top-most row.
