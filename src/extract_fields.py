...
import os, sys
from read_sat_images import read_sat_images_file
...
def get_polygons_by_municipal(polygons, municipal_nr):
    polygons_df = polygons.loc[polygons['KOMID'] == municipal_nr]['geometry']
    polygons_list = list(polygons_df)
...
if __name__ == '__main__':
    ...
    for municipal_nr in municipal_nrs:
        ...
        polygons_by_municipal_nr = get_polygons_by_municipal(polygons, municipal_nr)
        print(polygons_by_municipal_nr)
        ...
        # row = orgnrs_by_municipal_nr.loc[orgnrs_by_municipal_nr['commune_id'] == list(orgnrs_by_municipal_nr['commune_id'])[0]].iloc[0]
        # bounding_box = boundingBox(row['lat'], row['lng'], 1)
        
        # polygons_plt = [mapping(poly) for poly in polygons_by_municipal_nr]
        # polygons_plt = [poly['coordinates'][0][0] for poly in polygons_plt]
        # print(polygons_plt)
